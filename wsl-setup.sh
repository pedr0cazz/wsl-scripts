...
sudo -v
# Keep-alive to prevent sudo timeout
(while true; do
    sudo -n true
    sleep 60
done) &
...
# Persist config for helper scripts
echo "export WEB_ROOT=\"$WEB_ROOT\"" >~/.wsl_env
echo "export SSL_DIR=\"$SSL_DIR\"" >>~/.wsl_env
echo "export SCRIPTS_DIR=\"$SCRIPTS_DIR\"" >>~/.wsl_env
echo "export SSL_SCRIPT=\"\$SCRIPTS_DIR/ssl-manager.sh\"" >>~/.wsl_env
...
for STEP in $(seq 1 $TOTAL_STEPS); do
    ...
    1) # System update
        run_step "System update" bash -c 'sudo apt-get update -qq && sudo apt-get upgrade -qq'
        ;;
    ...
    5) # PHP
        run_step "Add PHP PPA & install PHP" bash -c 'sudo add-apt-repository -y ppa:ondrej/php >/dev/null 2>&1 || true; sudo apt-get update -qq; for v in 8.2 8.3 8.4; do sudo apt-get install -y -qq php${v} php${v}-fpm php${v}-cli; done'
        ;;
